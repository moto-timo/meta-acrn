...
require acrn-verified-boot-settings.inc
    ...
    dmverity_run () {
        ...
        while true; do
            ...
            if [ -e "$bootparam_root" ] && [ -e "$bootparam_dmverity" ]; then
                ...
                if ! veritysetup create "${ACRN_DEVICE_MAPPER_ROOTFS_NAME}" "$bootparam_root" "$bootparam_dmverity" "$roothash" --hash-offset "$headersize"; then
                    fatal "veritysetup of rootfs $bootparam_root using dm-verity hash partition $bootparam_dmverity failed"
                fi
                bootparam_root="/dev/mapper/${ACRN_DEVICE_MAPPER_ROOTFS_NAME}"
                return
            fi
...
python do_install () {
    ...
    with open(os.path.join(d.getVar('D'), 'init.d', '80-dmverity'), 'w') as f:
        f.write(d.getVar('acrn_dmverity'))

    privkey = d.getVar('ACRN_DMVERITY_PRIVATE_KEY')
    password = d.getVar('ACRN_DMVERITY_PASSWORD')
...
ACRN_DM_VERITY_KEY_NEEDED = "1"
inherit acrn-hash-dm-verity-key
do_install[vardeps] += "ACRN_DMVERITY_PRIVATE_KEY_HASH"
